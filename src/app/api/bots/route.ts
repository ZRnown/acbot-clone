...
import { verifyToken, getBotsByOwner, createBot, updateBot } from "@/lib/db";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!botToken) {
      return NextResponse.json({ error: "缺少必填字段: token" }, { status: 400 });
    }

    // Verify the token and auto-fetch bot info via KOOK API
    let botInfo;
    try {
